...
- `_template/article.html` を複製して記事を作る際は、テンプレート内の
  **全ての TODO コメント(og:url / og:image / canonical / 日付 / カテゴリ等)と
  プレースホルダを、必ず実データで置換・削除してから出力する**。
  TODO を残したまま出力しない(commit 前に `TODO` 文字列が残っていないか確認する)
